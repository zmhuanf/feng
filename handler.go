...
func serverHandle(s *server, isSys bool) func(c *gin.Context) {
...
func handleRequestBack(ctx IServerContext, s *server, req *request, isSys bool) error {
	...
	_, err := callServer(resp.fn, ctx, req.Data)
	if err != nil {
		resp.ch <- chanData{
			Success: false,
			Data:    err.Error(),
...
func handlePushOrRequest(ctx IServerContext, s *server, req *request, u *user, isSys bool) error {
	...
	for _, middleware := range middlewares {
		...
		_, err := callServer(middleware.fn, ctx, req.Data)
		if err != nil {
	...
	data, err := callServer(fn, ctx, req.Data)
	if err != nil {
		err = u.send(&request{
			ID:      req.ID,
			Type:    resType,
...
MAINFOR:
	for {
		...
		if req.Type == requestTypeRequestBack {
			...
			_, err = callClient(resp.fn, ctx, req.Data)
			if err != nil {
				resp.ch <- chanData{
					Success: false,
					Data:    err.Error(),
		...
		for _, middleware := range c.middlewares {
			...
			_, err = callClient(middleware.fn, ctx, req.Data)
			if err != nil {
		...
		data, err := callClient(fn, ctx, req.Data)
		if err != nil {
			err = c.send(&request{
				ID:      req.ID,
				Type:    resType,
